...
for i, temp in enumerate(temperature_list):
    ...
    # magnetization.dat
    magnet_src = os.path.join(output_dir, "magnetization.dat")
    magnet_dest = os.path.join(folder_path, f"magnet_{folder_T}.dat")
    if os.path.exists(magnet_src):
        shutil.move(magnet_src, magnet_dest)
        print(f"📁 Spostato {magnet_src} → {magnet_dest}")
    else:
        print(f"⚠️ magnetization.dat non trovato per T = {temp}")
