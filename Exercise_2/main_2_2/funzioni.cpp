...
vector<double> path_discrete(int Nstep, Random &rnd) {
    ...
    double x=0.0, y=0.0, z=0.0;  // posizione iniziale

    for (int n=0; n<Nstep; n++) {
        // --- scegli direzione casuale tra le 6 possibili
        int dir = int(6 * rnd.Rannyu());  // 0..5
        if (dir == 0) x += 1.0;
        if (dir == 1) x -= 1.0;
        if (dir == 2) y += 1.0;
        if (dir == 3) y -= 1.0;
        ...
        r2_walk[n] = x*x + y*y + z*z;  // aggiorno r^2 al passo n
    }
...
vector<double> path_continuous(int Nstep, Random &rnd) {
    ...
    for (int n = 0; n < Nstep; n++) {
        ...
        double u   = rnd.Rannyu(-1.0, 1.0);        // cos(theta)
        double phi = rnd.Rannyu(0.0, 2.0*M_PI);    // angolo azimutale
        double theta = acos(u);
        ...
        // aggiorna distanza quadrata
        r2_walk[n] = x*x + y*y + z*z;
    }
