...
private:
  ...
  double p_c=0.6;           //probability of crossover
  double p_m_swap=0.05;         //probability of swap of two genes
  double p_m_shift=0.05;       //probability of shift
  double p_m_exchange=0.05;    //probability of exchange two blocks //prima era 0.01 
  double p_m_block=0.05;       //probability of inversio ordered of a block //prima era 0.01
  ...
  field <Chromosome> _chromo_elite; // 
...
public: // Function declarations
  ...
  void initialize_parallel_rnd(int rank); //initialize n(depends on number of processor called) rnd primes and one seed for all process
  void initialize(int rank=0);          // Initialize system properties
  void initialize_properties(int rank=0); //read and initialize the file's property; (best_loss e avareged_block )
  ...
  void population(int rank=0);    //generate the chromosome population
  ...
  void sort_chromo_field(field <Chromosome> _exchange); //sort by loss the field in input
  ...
  void measure(int i, int rank);  //mesure and print best Loss and loss averaged on best half population;
  void read_city();   //read the city coordinates from a file

  void exchange_best_chromosomes(int rank, int size); //method to exchange cromos between process
};
