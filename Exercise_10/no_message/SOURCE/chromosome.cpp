...
void Chromosome :: check(){
    ...
    if(v.size() != _ndim ){
        ...
        cerr << "Errore: il vettore _g non è di dimensione _ndim.\n";
        exit(EXIT_FAILURE);
    }else if(_g(0) !=1 ){
...
void Chromosome :: exchange_block(int m){     
          //scambi due blocchi contigui di dimensione m< N-2
    if(m!=0){
        double M=double(_ndim)/2.0;
        if(m < M){    

       ivec sub1= _g.subvec(1,m);
       ivec sub2=_g.subvec(m+1,2*m) ;   
...
void Chromosome :: block_inversion(int m){   //inverte l'ordine di un blocco lungo m

if(m!=0 || m<_ndim-1){

   ivec sub= _g.subvec(1,m);
...
void  Chromosome :: set_gene(int k, int g){

    if(k<_ndim){ _g(k)=g;}
        else{ cout<<"PROBLEM: position "<< k <<" is out of bounds "<<endl ; }
