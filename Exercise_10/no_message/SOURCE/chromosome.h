...
public: // Function declarations
  ...
  void exchange_block(int m);                   //exhachange two sub m size's block, m< _ndim/2 
  void block_inversion(int m);                  // invert the ordern of a m size's block m<_ndim -1;
  void mix();                                  // mix the genes with suffle() respecting the bound;
  void set_loss(mat L);                      // compute and set the loss, by passing a mat;
  void set_gene(int k, int g);              // sets the allele g in the k-position 
