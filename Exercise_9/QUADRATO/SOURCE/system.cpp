  ...
  ifstream input("../INPUT/properties.dat");
    ...
    while ( !input.eof() ){
      ...
      if( property == "LOSS_GENERATION" ){
        ...
        ofstream coutl("../OUTPUT/loss_gen.dat");
        coutl.close();
        
      } else if( property == "AVERAGED_LOSS" ){

        ofstream coutm("../OUTPUT/mean_loss.dat");
        coutm.close();
       
      }  else if( property == "PATH" ){

        ofstream coutpath("../OUTPUT/path.dat");
        coutpath.close();
...
void System :: population(){    //generate the population, initialize the chromo_son field and the best_chromo
...
void System :: measure(int i){
  //best loss
  ofstream coutl;
  coutl.open("../OUTPUT/loss_gen.dat",ios::app);
  ...
  ofstream coutm;
  coutm.open("../OUTPUT/mean_loss.dat",ios::app);
  ...
  if(i==_ngen-1){
    ...
    ofstream coutpath;
    coutpath.open("../OUTPUT/path.dat",ios::app);
    int riga;
    for(int k=0; k<_n_city; k++){
