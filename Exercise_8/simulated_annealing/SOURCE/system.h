...
private:
  ...
  string _input_velocities; //add by me 
  string _start_velocities; // Starting distribution of velocities
  int _npart;           // Number of particles
  //VMC
  double _x0;           //ADD BY ME; starting position for VMC
  double _mu;           //ADD BY ME; mean value for  gaussian autofunction  exp(-(x- mu)/2 sigma^2)
  double _sigma;        //ADD BY ME; standard deviation value for  gaussian autofunction
  //SIMULATED ANNEALING
  double _inverse_dt;   //ADD BY ME; decreasing of temperature at each simulated annealing step
  double _delta_mu;     //ADD BY ME; metropolis step for mu
  double _delta_sigma;  //ADD BY ME; metropolis step for sigma
  int _sa_steps;        //ADD BY ME; numebers of steps at each temperature 
  ...
  bool _measure_instant_temp;                               // ADD BY ME ; Flag for measuring instant temperature
  bool _measure_instant_magnet;                             // ADD BY ME ; Flag for measuring instant magnetization;
  bool _measure_instant_pot;                                // ADD BY ME ; Flag for measuring instant potential;
  bool _measure_local_energy;                               // ADD BY ME ; Flag for measuring local energy in VMC
  ...
  int _index_cvH2;                                          //ADD BY ME. Indices for accessing H2, useful for compute c_v
  int _index_pofv;                                          // Index for accessing velocity modulus distribution
  int _index_local_energy;                                  // ADD BY ME. Indices for accessing local energy in VMC
...
public: // Function declarations
  ...
  double get_temp();          // ADD by me ; Get the temperature
  double get_inverse_dt();    // ADD BY ME; Get the inverse_temperature
  int get_sa_steps();         // ADD by me ; GEt the number of steps at each tempearture in SA
  void initialize();          // Initialize system properties
  void initialize_properties();// Initialize properties for measurement
  void write_starting_velocities_distribution(); // add by me Write starting velocities distribution
  ...
  void spin(); //aggiunto dame per stampare gli spin a schermo, per vedere se inizializza tutti gli spin a 1;
  //Method used in VMC
  double psi2_trial(double x); //Add by me; This method compute square modulus of psi_trisal= exp()+exp() me it's the ansatz !!!IT'S A NON NORMALIZE PROBABILITY
  double local_energy(double x); //Add by me; This method compute H[psi_t]/psi_t
  void move_vmc();

  void print_position(int blk);//Add by me ; debug vmc
  double run_vmc(double mu, double sigma); //Add by me ; This method compute a VMC for SA
  void simulated_annealing(double T); //ADD BY ME; This method compute a SA; ritorna l'energia locale progressiva all'ultimo blocco
