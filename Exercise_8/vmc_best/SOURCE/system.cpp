...
void System::move_vmc() {
    ...
    double psi_old = psi2_trial(_particle(0).getposition(0, false)); // x_old
    double psi_new = psi2_trial(_particle(0).getposition(0, true));  // x_trial

    double acceptance = psi_new / psi_old;
...
void System :: initialize_properties(){ // Initialize data members used for measurement of properties
  ...
  _measure_cv       = false;
  _measure_chi      = false;
  _measure_instant_temp = false; //aggiunto per misurare la temperatura istantanea;
  _measure_instant_magnet= false; //aggiunto per misurare la magnetizazzione istantanea;
  _measure_instant_pot= false;    //aggiunto da me
  _measure_local_energy=false;    //add by me 
  _measure_prob_density= false;   //add by me
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        coutle << "#     BLOCK:   ACTUAL_LE:     X_LE:       ERROR:" << endl;
        coutle.close();
        _nprop++;
      ...
      } else if (property == "PROB_DENSITY") {
        
        ofstream coutp("../OUTPUT/prob_density.dat");
        coutp << "# block   x   hist_block   hist_prog   err" << endl;
        coutp.close();
        input >> _n_bins_dp;              // numero di bin per l'istogramma
        _nprop += _n_bins_dp;
        
        _bin_size_dp = (_x_max - _x_min) / double(_n_bins_dp); // definisco range in input
        _measure_prob_density = true;
        _index_prob_density = index_property;
        index_property += _n_bins_dp;
...
if(_measure_pofv){ 
  ...
  */
  ...
  //PROB DENSITY VMC
  if (_measure_prob_density) {
    double x = _particle(0).getposition(0, true);
    int bin = int((x - _x_min) / _bin_size_dp);
    if (bin >= 0 && bin < _n_bins_dp) {
        _measurement[_index_prob_density + bin] += 1.0;
    }
}
...
if (_measure_prob_density) {
      ...
      int k;
      k=_nblocks-blk;
    if(k==0){ //write the last distribution
    ofstream coutp("../OUTPUT/prob_density.dat", ios::app);

    for (int i = 0; i < _n_bins_dp; i++) {
        int idx = _index_prob_density + i;

        // Media del blocco per questo bin
        double average = _average(idx);

        // Medie globali accumulate
        double sum_average = _global_av(idx);
        double sum_ave2    = _global_av2(idx);

        // Coordinate del centro del bin
        double x = _x_min + (i + 0.5) * _bin_size_dp;

        // Normalizzazione: densità = conteggi / (bin_width)
        double norm_block = average / _bin_size_dp;
        double norm_prog  = (sum_average / blk) / _bin_size_dp;
        double err        = this->error(sum_average, sum_ave2, blk) / _bin_size_dp;
