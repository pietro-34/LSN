...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    /*  if( _sim_type == 0){
      input >> _input_velocities; //leggo distribuzione iniziale velocità;
      
          if(_input_velocities!="MAXBOLTZ" && _input_velocities!="DIRAC" ){
            coutf<< "INITIALIZE VELOCITIES DISTRIBUTION FROM PREVIUS SIMULATION"<<endl;
            _restart=1;
          }
            else{
                    _start_velocities=_input_velocities;
                    coutf << "STARTING VELOCITIES DISTRIBUTION=  "<< _start_velocities << endl;}  //stampo su output.dat la distribuzione di velocità;
      }*/
      if(_sim_type > 3){
    ...
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        //index_property++;
         ...
         _ptail = (32.0/3.0)*M_PI*_rho*((1.0/(3.0*pow(_r_cut,9)))-(1.0/(2*pow(_r_cut,3)))); //FIXED IN EXERCISE 7---> natural units  32/3*Pi*rho(1/3*r_c^9 - 1/r_c^3)
...
if(_measure_pofv){
  ...
  if(_nblocks-blk==0){ //stampo solo l'ultimo blocco
    ...
    for(int i=0; i<_n_bins; ++i){
        ...
        // !!!ATTENZIONEEE!!! in initaliaze properties definisco un file dove stampo "# BLOCK:  DISTANCE:     GOFR_BLOCK: GOFR_PROG ERROR:"
        coutf << setw(12)<< blk
              << setw(12)<< r
              << setw(12)<< g_block
              << setw(12)<< g_prog
