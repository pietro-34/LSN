...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      if( _sim_type == 0 ){
      input >> _start_velocities; //leggo distribuzione iniziale velocità;
    ...
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        //index_property++;
         ...
         _ptail = (32.0/3.0)*M_PI*_rho*((1.0/(3.0*pow(_r_cut,9)))-(1.0/(2*pow(_r_cut,3)))); //FIXED IN EXERCISE 7---> natural units  32*Pi*rho(1/3*r_c^9 - 1/2*r_c^3)
